...
class Seq2SeqAttentionModel(object):
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('decoder'), tf.device(self._next_device()):
                ...
                decoder_outputs, self._dec_out_state, self._attention_list = seq2seq.attention_decoder(
    ...
    def decode_topk(self, sess, latest_tokens, enc_top_states, dec_init_states):
        ...
        results = sess.run(
            [self._topk_ids, self._topk_log_probs, self._dec_out_state, self._attention_list],
            feed_dict=feed)

        ids, probs, states, attention = results[0], results[1], results[2], results[3][0][0]
        # print len(results[3]),len(results[3][0])
        # print results[3]
        new_states = [s for s in states]
        return ids, probs, new_states, attention
