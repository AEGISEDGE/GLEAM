...
tf.app.flags.DEFINE_string('concept_key', 'concept',
                           'tf.Example feature key for concept.')
tf.app.flags.DEFINE_string('topic_key', 'topic',
                           'tf.Example feature key for topic.')
...
def _RunningAvgLoss(loss, running_avg_loss, summary_writer, step, decay=0.999):
    ...
    sys.stdout.write('epoch: %d, running_avg_loss: %f\n' % (step, running_avg_loss))
...
def _Train(model, data_batcher):
    ...
    with tf.device('/cpu:0'):
        ...
        sess = sv.prepare_or_wait_for_session(config=gpu_config)
        running_avg_loss = 0
        step = 0
        while not sv.should_stop() and step < FLAGS.max_run_steps:
            (article_batch, abstract_batch, topic_batch, targets, article_lens, abstract_lens,
             loss_weights, _, _) = data_batcher.NextBatch()
            (_, summaries, loss, train_step) = model.run_train_step(
                sess, article_batch, abstract_batch, topic_batch, targets, article_lens,
...
def main(unused_argv):
    ...
    hps = seq2seq_attention_model.HParams(
        ...
        enc_timesteps=60,
        dec_timesteps=30,
        min_input_len=7,  # discard articles/summaries < than this
        num_hidden=256,  # for rnn cell
        emb_dim=128,  # If 0, don't use embedding
        max_grad_norm=2,
        num_softmax_samples=4096)  # If 0, no sampled softmax.
    if hps.mode == 'decode':
        function_name = dec_reader
    else:
        function_name = batch_reader
    batcher = function_name.Batcher(
        FLAGS.data_path, vocab, hps, FLAGS.article_key,
        FLAGS.abstract_key, FLAGS.topic_key, FLAGS.max_article_sentences,
