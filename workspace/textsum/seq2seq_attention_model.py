...
class Seq2SeqAttentionModel(object):
    ...
    def run_train_step(self, sess, article_batch, abstract_batch, topic_batch, targets,
        ...
        return sess.run(to_return,
                        feed_dict={self._articles: article_batch,
                                   ...
                                   self._topic_vectors: topic_batch,
                                   self._targets: targets,
                                   self._article_lens: article_lens,
                                   self._abstract_lens: abstract_lens,
                                   self._loss_weights: loss_weights})

    def run_eval_step(self, sess, article_batch, abstract_batch, topic_batch, targets,
                      article_lens, abstract_lens, loss_weights):
    ...
    def run_decode_step(self, sess, article_batch, abstract_batch, topic_batch, targets,
                        article_lens, abstract_lens, loss_weights):
    ...
    def _add_placeholders(self):
        ...
        self._topic_vectors = tf.placeholder(tf.float32,
                                             [hps.batch_size, hps.num_hidden])
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('topic_vector'):
                w_fw = tf.get_variable(
                    'w_fw', [hps.num_hidden, hps.num_hidden], dtype=tf.float32,
                    initializer=tf.truncated_normal_initializer(stddev=1e-4))
                w_bw = tf.get_variable(
                    'w_bw', [hps.num_hidden, hps.num_hidden], dtype=tf.float32,
                    initializer=tf.truncated_normal_initializer(stddev=1e-4))
                bias_fw = tf.get_variable(
                    'bias_fw', [hps.num_hidden], dtype=tf.float32,
                    initializer=tf.truncated_normal_initializer(stddev=1e-4))
                bias_bw = tf.get_variable(
                    'bias_bw', [hps.num_hidden], dtype=tf.float32,
                    initializer=tf.truncated_normal_initializer(stddev=1e-4))

            topic_vector_fw = tf.matmul(self._topic_vectors, w_fw) + bias_fw
            topic_vector_fw = tf.tanh(topic_vector_fw)
            topic_vector_bw = tf.matmul(self._topic_vectors, w_bw) + bias_bw
            topic_vector_bw = tf.tanh(topic_vector_bw)
            topic_vector_final = tf.concat(1, [topic_vector_fw, topic_vector_bw])
            ...
            encoder_outputs = [hidden_state * topic_vector_final
                               for hidden_state in emb_encoder_inputs]
    ...
    def _add_train_op(self):
        ...
        self._lr_rate = tf.maximum(
            ...
            tf.train.exponential_decay(hps.lr, self.global_step, 30000, 0.95))
    ...
    def encode_top_state(self, sess, enc_inputs, topic_vector, enc_len):
        """Return the top states from encoder for decoder.

        Args:
          sess: tensorflow session.
          enc_inputs: encoder inputs of shape [batch_size, enc_timesteps].
          topic_vector: encoder topic vector [batch_size, hidden_state]
        ...
        results = sess.run([self._enc_top_states, self._dec_in_state],
                           feed_dict={self._articles: enc_inputs,
                                      ...
                                      self._topic_vectors: topic_vector})
