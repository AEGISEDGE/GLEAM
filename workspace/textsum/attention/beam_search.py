...
class Hypothesis(object):
    ...
    def __init__(self, tokens, log_prob, state, attention):
    ...
    def Extend(self, token, log_prob, new_state, attention):
        ...
        return Hypothesis(self.tokens + [token], self.log_prob + log_prob,
                          new_state, self.attentions + [attention])
...
class BeamSearch(object):
    ...
    def BeamSearch(self, sess, enc_inputs, enc_seqlen):
        ...
        while steps < self._max_steps and len(results) < self._beam_size:
            ...
            topk_ids, topk_log_probs, new_states, attentions = self._model.decode_topk(
            ...
            for i in xrange(num_beam_source):
                ...
                for j in xrange(self._beam_size * 2):
                    all_hyps.append(h.Extend(topk_ids[i, j], topk_log_probs[i, j], ns, attentions[i]))
