...
ModelInput = namedtuple('ModelInput',
                        'enc_input dec_input topic_vector target enc_len dec_len '
...
class Batcher(object):
    ...
    def NextBatch(self):
        ...
        topic_batch = np.zeros((self._hps.batch_size, self._hps.num_hidden), dtype=np.float32)

        buckets = self._bucket_input_queue.get()
        for i in xrange(self._hps.batch_size):
            (enc_inputs, dec_inputs, topic_vector, targets, enc_input_len, dec_output_len,
            ...
            topic_batch[i, :] = topic_vector[:]
            for j in xrange(dec_output_len):
                loss_weights[i][j] = 1
        return (enc_batch, dec_batch, topic_batch, target_batch, enc_input_lens, dec_output_lens,
    ...
    def _FillInputQueue(self):
        ...
        while True:
            ...
            for i in xrange(min(self._max_abstract_sentences,
                ...
                dec_inputs += data.GetWordIds_abstract(abstract_sentences[i], self._vocab)
            ...
            element = ModelInput(enc_inputs, dec_inputs, topic_vector,
                                 targets, enc_input_len,
